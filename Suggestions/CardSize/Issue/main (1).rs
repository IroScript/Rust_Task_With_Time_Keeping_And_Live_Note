...
impl Default for TextStyleConfig {
    fn default() -> Self {
        Self {
            ...
            main_line_gap: 5.0,    // 0–50 scale: 5.0 ≈ 1.18× line height (comfortable default)
            sub_line_gap: 5.0,     // 0–50 scale: 5.0 ≈ 1.18× line height
...
fn layout_quote_text(
    ...
    default_size: f32,
    wrap_width: f32,
    line_gap: f32,  // 0.0–50.0: 0=lines overlap 2%, 1≈touch, 5≈normal(1.18x), 50=3x spacing
) -> Arc<egui::Galley> {
    // Convert 0–50 gap value to an actual line_height in pixels.
    // 0   → 0.98 × font_size  (2% overlap: previous line bottom bleeds into next)
    // 0.5 → 1.00 × font_size  (lines just touching, zero gap)
    // 5.0 → ~1.18× font_size  (comfortable default, similar to old 1.2 multiplier)
    // 50  → 3.00 × font_size  (very spacious)
    let gap_clamped = line_gap.clamp(0.0, 50.0);
    let line_height_px = default_size * (0.98 + gap_clamped * (3.0 - 0.98) / 50.0);
    ...
    // Slow path — character-level formatting.
    ...
    for i in 1..=char_count {
        ...
        if cur_format != run_format {
            ...
            // Per-run line_height: scale proportionally to run's font size
            let run_line_h = size * (0.98 + gap_clamped * (3.0 - 0.98) / 50.0);
            job.append(
                &run_text,
                0.0,
                egui::TextFormat {
...
fn render_quote_card(
    ...
    let (_quote_main, quote_sub, text_style, is_new_quote) = if let Some(idx) = idx_opt {
        ...
        (state.main_text_input.clone(), state.sub_text_input.clone(), state.text_style.clone(), true)
    };
    ...
    // Apply card scale to height
    let card_scale = state.card_scale;
    let card_max_height = 450.0 * card_scale;
    ...
    let card_resp = if card_scale <= 0.001 {
        ...
        ui.allocate_ui_with_layout(
            ...
            |ui| {
                // Set hard clip on this ui context
                let clip_rect = ui.clip_rect();
                let mut new_clip = clip_rect;
                new_clip.max.y = new_clip.min.y + card_max_height;
            ...
            // desired_rows = 1 always: ScrollArea's max_height controls visible area,
            // not TextEdit's minimum. This prevents TextEdit from forcing card taller.
            let dynamic_rows = 1usize;
            
            // Clear scroll state if this card is NOT the active scroll card
            ...
            if !is_active_card {
                // Reset ONLY the scroll OFFSET to top, without removing content_size.
                // Removing the entire State causes content_size = 0 → ScrollArea collapses to 0px.
                let scroll_id = id.with("scroll_main");
                ui.ctx().data_mut(|d| {
                    if let Some(scroll_state) = d.get_temp_mut::<egui::scroll_area::State>(scroll_id) {
                        scroll_state.offset.y = 0.0;
                    }
                });
            }
            ...
            let out = egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    egui::TextEdit::multiline(&mut edit_text)
                        .id(edit_id)
                        .desired_rows(dynamic_rows)  // Always 1 — ScrollArea max_height controls visible rows
                        .desired_width(card_width)   // Full width - no horizontal margin
                        .margin(Vec2::ZERO)           // Remove internal TextEdit padding — no wasted space
                        .font(FontId::proportional(main_size))
                        .text_color(u32_to_color32(main_color))
                        .frame(false)
                        .layouter(&mut |ui, text, wrap_width| {
                            layout_quote_text(ui, text, &main_formats, u32_to_color32(main_color), main_size, wrap_width, text_style.main_line_gap)
                        })
                        .show(ui)
                });
            ...
            if out.response.changed() {
                if is_new_quote {
                    ...
                    state.save(); // Auto-save on every keypress as requested
                }
            }
            ...
            if !quote_sub.is_empty() && card_scale > 0.05 {
                // Between-gap: scale with card so it shrinks proportionally at small sizes
                let between_px = text_style.between_gap * card_scale;
                if between_px > 0.5 {
                    ui.add_space(between_px);
                }
                ...
                // Gap after separator before sub text
                if between_px > 0.5 {
                    ui.add_space(between_px);
                }
                ...
                // Clear scroll state for sub text if not active card
                if !is_active_card {
                    let sub_scroll_id = id.with("scroll_sub");
                    ui.ctx().data_mut(|d| {
                        if let Some(s) = d.get_temp_mut::<egui::scroll_area::State>(sub_scroll_id) {
                            s.offset.y = 0.0;
                        }
                    });
                }
                ...
                let out_sub = egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        egui::TextEdit::multiline(&mut edit_sub_text)
                            .id(edit_id)
                            .desired_rows(sub_rows)       // Always 1 — ScrollArea handles visible area
                            .desired_width(card_width)    // Full width - no horizontal margin
                            ...
                            .frame(false)
                            .layouter(&mut |ui, text, wrap_width| {
                                layout_quote_text(ui, text, &sub_formats, u32_to_color32(sub_color), sub_size, wrap_width, text_style.sub_line_gap)
                            })
                            .show(ui)
                    });
        ...
        let out = egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                egui::TextEdit::multiline(&mut edit_text)
                    ...
                    .layouter(&mut |ui, text, wrap_width| {
                        layout_quote_text(ui, text, &main_formats, u32_to_color32(main_color), main_size, wrap_width, state.text_style.main_line_gap)
                    })
                    .show(ui)
            }).inner;
            ...
            let out_sub = egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    egui::TextEdit::multiline(&mut edit_sub_text)
                        ...
                        .return_key(None) // Don't consume Enter key
                        .layouter(&mut |ui, text, wrap_width| {
                            layout_quote_text(ui, text, &sub_formats, u32_to_color32(sub_color), sub_size, wrap_width, state.text_style.sub_line_gap)
                        })
                        .show(ui)
                }).inner;
        ...
        .show(ctx, |ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    // Click on empty area saves current editing but does NOT clear input fields
                    if bg_resp.clicked() && state.editing_quote_index.is_some() {
                        // Save current editing
                        state.save();
                        // Do NOT clear main_text_input or sub_text_input
                        // User text must persist in input fields
...
pub fn render_control_panel_contents(
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            // 1. Omni-Drag detection for sideboard (handles window moving when clicking gaps)
            if state.drag_anywhere_enabled {
                if ui.input(|i| i.pointer.primary_down()) && ui.ctx().dragged_id().is_none() {
            ...
            render_section(ui, &format!("ADD CUSTOM TEXT  [{}]", state.quotes.len() + 1), u32_to_color32(state.text_style.panel_text_color), |ui| {
                ...
                // --- Main text input (same width as Add Text button) ---
                // --- Main text input (simple, no layouter) ---
                let text_width = (section_content_width - 80.0).max(50.0);
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        let resp = egui::ScrollArea::vertical()
                            .max_height(200.0)
                            .id_salt("panel_main_input_scroll")
                            .auto_shrink([false, true])
                            .show(ui, |ui| {
                                ui.add(
                                    egui::TextEdit::multiline(&mut state.main_text_input)
                                        .hint_text("Main text... (Enter for new line)")
                                        .desired_rows(3)
                        ...
                        // Check if this input field was clicked BEFORE moving resp
                        if resp.clicked() {
                            main_text_clicked = true;
                        }
                ...
                // --- Supporting text input (simple, no layouter) ---
                    ...
                    .show(ui, |ui| {
                        let resp = ui.add(
                            egui::TextEdit::multiline(&mut state.sub_text_input)
                                .hint_text("Supporting text... (Enter for new line)")
                                .desired_rows(2)
                                .desired_width(text_width)
                                .frame(false),
                        );
                ...
                // Add button - ULTRA SENSITIVE: any click anywhere shows the card
                let add_btn_color = Color32::from_rgb(76, 175, 80);
                let _add_response = draw_text_button(
                    ui,
                    "+ Add Text",
                    add_btn_color,
                ...
                .on_hover_text("Add New Quote\nCreates a new motivational quote entry\nClick anywhere to show as top card");
                
                // Show card on any click anywhere EXCEPT on the input fields themselves
                if ui.input(|i| i.pointer.any_click()) && !main_text_clicked && !sub_text_clicked {
                    state.save_current_input();
                }
            });
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        if ui
                            .add_sized(
                                ...
                                egui::Slider::new(&mut main_gap, 0.0_f32..=50.0)
                                    .step_by(0.5)
                                    .text(""),
                            )
                            .changed()
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        if ui
                            .add_sized(
                                [slider_width, ui.available_height()],
                                egui::Slider::new(&mut sub_gap, 0.0_f32..=50.0)
                                    .step_by(0.5)
                                    .text(""),
                            )
                            .changed()
...
impl AppRunner {
    fn render(&mut self, window: &Window) {
        ...
        let full_output = egui_ctx.run(raw_input, |ctx| {
            ...
            // Outside click: save in-progress editing, then clear inputs
            // Only clear if the click was on EMPTY SPACE — not on a card.
            // card_was_clicked flag is set by card handlers earlier this same frame.
            if ctx.input(|i| i.pointer.any_click()) {
                // Check if any TextEdit has focus
                let has_text_focus = ctx.memory(|mem| mem.focused().is_some());
                
                // Check if any card or UI element was clicked
                let clicked_on_ui = ctx.memory(|mem| mem.any_popup_open()) 
                                 || ctx.is_pointer_over_area();
                
                // Only clear if: no text focus AND not clicking on any UI element AND not a card click
                if !has_text_focus && !clicked_on_ui && !app_state.card_was_clicked {
                    if app_state.editing_quote_index.is_some() {
                        // Editing existing card — save in place (BUG 3: no move to top)
                        app_state.save();
                        app_state.editing_quote_index = None;
                    } else if !app_state.main_text_input.trim().is_empty() {
                        // Was typing NEW text — save if not duplicate
                        let main = app_state.main_text_input.trim().to_string();
                        let sub  = app_state.sub_text_input.trim().to_string();
                        let already_exists = app_state.quotes.iter().any(|q| {
                            q.main_text.trim() == main.as_str() && q.sub_text.trim() == sub.as_str()
                        });
                        if !already_exists {
                            app_state.add_quote(main, sub);
                        }
                    }
                    // Clear inputs only on genuine empty-space clicks
