...
fn render_formatted_text(
    ...
    if formats.is_empty() {
        ...
        for i in 0..chars.len() {
            let ch = chars[i];
            let format = formats.get(i).cloned().unwrap_or_default();
            
            if i > 0 && format != current_format {
                let color = if let Some(c) = current_format.color {
                    Color32::from_rgba_unmultiplied(c[0], c[1], c[2], c[3])
                } else {
                    default_color
                };
                let size = current_format.size.unwrap_or(default_size);
    ...
    let card_resp = ui.allocate_ui_with_layout(
        ...
        |ui| {
            ...
            ui.set_min_height(120.0);
                ...
                // Dynamic height mode when editing - expands with content
                ui.add_space(10.0);
            ...
            let out = egui::ScrollArea::vertical()
                ...
                .max_height(main_size * 4.5)
                .auto_shrink([false, true])
                .show(ui, |ui| {
                    egui::TextEdit::multiline(&mut edit_text)
                        .id(edit_id)
                        .desired_rows(3)
                        .desired_width(card_width - 16.0)
                        .font(FontId::proportional(main_size))
                        .text_color(u32_to_color32(main_color))
                        .frame(false)
                        .layouter(&mut |ui, text, wrap_width| {
                            layout_quote_text(ui, text, &main_formats, u32_to_color32(main_color), main_size, wrap_width)
                        })
                        .show(ui)
                }).inner;
            ...
            if out.response.changed() {
                if is_new_quote {
                    ...
                    let i = idx.unwrap();
                    state.quotes[i].main_text = edit_text.clone();
                ...
                let out_sub = egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        egui::TextEdit::multiline(&mut edit_sub_text)
                            ...
                            .frame(false)
                            .layouter(&mut |ui, text, wrap_width| {
                                layout_quote_text(ui, text, &sub_formats, u32_to_color32(sub_color), sub_size, wrap_width)
                            })
                            .show(ui)
                    }).inner;
...
pub fn render_control_panel_contents(
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            // Compute panel_content_width INSIDE the ScrollArea so it reflects the
            // actual available width AFTER the scrollbar has been reserved.
            // Using a static constant caused overflow when the scrollbar was visible.
            let panel_content_width = ui.available_width().max(0.0);
            ...
            render_section(ui, &format!("ADD CUSTOM TEXT  [{}]", state.quotes.len() + 1), u32_to_color32(state.text_style.panel_text_color), |ui| {
                ...
                ui.horizontal(|ui| {
                    // Textarea on the left — width derived from actual available space
                    ...
                    egui::Frame::none()
                        ...
                        .show(ui, |ui| {
                            // Get formats for main text if editing an existing quote
                            let formats = if let Some(edit_idx) = state.editing_quote_index {
                                if edit_idx < state.quotes.len() {
                                    state.quotes[edit_idx].main_text_formats.clone()
                                } else {
                                    Vec::new()
                                }
                            } else {
                            ...
                            let main_color = u32_to_color32(state.text_style.main_text_color);
                            let main_size = state.text_style.main_text_size;

                            let resp = egui::ScrollArea::vertical()
                                .id_salt("main_input_scroll")
                                .max_height(100.0)
                                .auto_shrink([false, true])
                                .show(ui, |ui| {
                                    egui::TextEdit::multiline(&mut state.main_text_input)
                                        ...
                                        .frame(false)
                                        .layouter(&mut |ui, text, wrap_width| {
                                            layout_quote_text(ui, text, &formats, main_color, main_size, wrap_width)
                                        })
                                        .show(ui)
                                }).inner;
                    ...
                    let text_out = text_response.unwrap();
                    ...
                    // Track selection for main text in control panel
                    if let Some(edit_idx) = state.editing_quote_index {
                        check_selection_and_position_toolbar(
                            ui, &text_out.response, &text_out.galley, text_out.galley_pos, egui::Id::new("main_input_scroll"), edit_idx, true, state
                        );
                    }
                ...
                ui.horizontal(|ui| {
                    ...
                    egui::Frame::none()
                        ...
                        .rounding(Rounding::same(4.0))
                        .show(ui, |ui| {
                            // Get formats for sub text if editing an existing quote
                            let formats = if let Some(edit_idx) = state.editing_quote_index {
                                if edit_idx < state.quotes.len() {
                                    state.quotes[edit_idx].sub_text_formats.clone()
                                } else {
                                    Vec::new()
                                }
                            } else {
                            ...
                            let sub_color = u32_to_color32(state.text_style.sub_text_color);
                            let sub_size = state.text_style.sub_text_size;

                            let resp = egui::ScrollArea::vertical()
                                .id_salt("sub_input_scroll")
                                .max_height(65.0)
                                .auto_shrink([false, true])
                                .show(ui, |ui| {
                                    egui::TextEdit::multiline(&mut state.sub_text_input)
                                        ...
                                        .frame(false)
                                        .layouter(&mut |ui, text, wrap_width| {
                                            layout_quote_text(ui, text, &formats, sub_color, sub_size, wrap_width)
                                        })
                                        .show(ui)
                                }).inner;
                    ...
                    let sub_out = sub_response.unwrap();
                    if sub_out.response.changed() {
                        // Autosave on every keypress
                        if let Some(edit_idx) = state.editing_quote_index {
                            // Editing existing quote - update it
                            if edit_idx < state.quotes.len() {
                    ...
                    // Track selection for sub text in control panel
                    if let Some(edit_idx) = state.editing_quote_index {
                        check_selection_and_position_toolbar(
                            ui, &sub_out.response, &sub_out.galley, sub_out.galley_pos, egui::Id::new("sub_input_scroll"), edit_idx, false, state
                        );
                    }
            ...
            render_section(ui, &format!("TEXT LIST ({})", state.quotes.len()), u32_to_color32(state.text_style.panel_text_color), |ui| {
                ...
                let list_box_internal_width = (ui.available_width() - 28.0).max(10.0);
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if avail > 8.0 {
                            let (line_rect, _) =
                                ui.allocate_exact_size(Vec2::new(avail - 6.0, 1.0), Sense::hover());
                            let mid_y = line_rect.center().y;
                            ui.painter().line_segment(
                                [
                                    egui::pos2(line_rect.left() + 2.0, mid_y),
                                    egui::pos2(line_rect.right() - 2.0, mid_y),
                                ],
