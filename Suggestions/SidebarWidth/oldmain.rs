    ...
    fn default() -> Self {
        ...
        if let Some(config) = AppConfig::load() {
            ...
            Self {
                ...
                hover_edit_enabled: true, // Default to enabled
                small_window_custom_popup_open: false,
                small_window_custom_popup_pos: None,
                editing_quote_index: None,
                pending_edit_caret: None,
...
fn render_format_toolbar(ctx: &egui::Context, state: &mut AppState) {
    ...
    egui::Window::new("Format")
        ...
        .resizable(false)
        .collapsible(false)
        .fixed_pos(egui::pos2(100.0, 100.0)) // Fixed position for now
        .show(ctx, |ui| {
            ui.horizontal(|ui| {
...
fn render_formatted_text(
    ...
    if formats.is_empty() || text.is_empty() {
        // No formatting, render normally
        ui.label(RichText::new(text).color(default_color).size(default_size));
        return;
    }
    
    // Render character by character with formatting
    ui.horizontal_wrapped(|ui| {
        ui.spacing_mut().item_spacing.x = 0.0; // No spacing between characters
        
        let default_format = CharFormat::default();
        
        for (i, ch) in text.chars().enumerate() {
            let format = formats.get(i).unwrap_or(&default_format);
            
            // Get color (use custom or default)
            let color = if let Some(color_array) = format.color {
            ...
            // Get size (use custom or default)
            let size = format.size.unwrap_or(default_size);
            
            // Render the character
            ui.label(RichText::new(ch.to_string()).color(color).size(size));
        }
    });
}
...
fn render_quote_card(
    ...
    // Ensure minimum width to prevent assertion failure in egui
    let card_width = (available * 0.9796).max(1.0);
    ...
    // Check if this card is being edited (has focus or is the editing quote)
    let is_being_edited = if let Some(idx) = idx_opt {
        state.editing_quote_index == Some(idx) || 
        ui.ctx().memory(|m| m.has_focus(id.with("edit_main")) || m.has_focus(id.with("edit_sub")))
    } else {
        !state.main_text_input.is_empty() || !state.sub_text_input.is_empty()
    };
    ...
    let card_resp = ui.allocate_ui_with_layout(
        ...
        |ui| {
            ...
            if !is_being_edited {
                // Fixed height mode when NOT editing
                ui.set_height(120.0);
            ...
            // Check if this text has formatting
            let has_formatting = if !is_new_quote {
                let idx = idx_opt.unwrap();
                !state.quotes[idx].main_text_formats.is_empty()
            } else {
                false
            };
            
            // ALWAYS render as TextEdit to prevent height changes
            let edit_id = id.with("edit_main");
            
            // Check if this field has focus
            let has_focus = ui.ctx().memory(|m| m.has_focus(edit_id));
            
            // If has formatting and no focus, show formatted text
            // Otherwise show editable TextEdit
            if has_formatting && !has_focus && !is_new_quote {
                let idx = idx_opt.unwrap();
                let text = &state.quotes[idx].main_text;
                let formats = &state.quotes[idx].main_text_formats;
                
                // Render formatted text
                let formatted_response = ui.vertical_centered(|ui| {
                    render_formatted_text(
                        ui,
                        text,
                        formats,
                        u32_to_color32(main_color),
                        main_size,
                    );
                });
                
                // Make it clickable to enter edit mode
                if formatted_response.response.clicked() {
                    ui.ctx().memory_mut(|m| m.request_focus(edit_id));
                }
            } else {
                // Editable mode with TextEdit
                let mut edit_text = if is_new_quote {
                    state.main_text_input.clone()
                } else {
                    state.quotes[idx_opt.unwrap()].main_text.clone()
                };

                let out = egui::TextEdit::multiline(&mut edit_text)
                    .id(edit_id)
                    .desired_rows(3)
                    .desired_width(card_width - 32.0)
                ...
                if !is_new_quote && out.response.has_focus() {
                    if let Some(text_edit_state) = egui::TextEdit::load_state(ui.ctx(), edit_id) {
                        if let Some(cursor_range) = text_edit_state.cursor.char_range() {
                            ...
                            if start != end {
                                ...
                                // Clear selection if cursor is just positioned
                                if let Some(sel) = &state.char_selection {
                                    if sel.is_main_text && sel.quote_index == idx_opt.unwrap() {
                ...
                if state.hover_edit_enabled {
                    ...
                    if is_hovering_main_text {
                        ...
                        // Determine if we should surrender focus
                        let is_dragged = out.response.dragged();
                        
                        // If dragged, trigger window drag and surrender focus
                        if is_dragged {
                            let _ = _window.drag_window();
                    ...
                    // Text is always ready to accept input when clicked
            ...
            if !quote_sub.is_empty() {
                ...
                let out_sub = egui::TextEdit::multiline(&mut edit_sub_text)
                    .id(edit_id)
                    .desired_rows(2)
                    .desired_width(card_width - 32.0)
                ...
                if !is_new_quote && out_sub.response.has_focus() {
                    if let Some(text_edit_state) = egui::TextEdit::load_state(ui.ctx(), edit_id) {
                        if let Some(cursor_range) = text_edit_state.cursor.char_range() {
                            let start = cursor_range.primary.index.min(cursor_range.secondary.index);
                            let end = cursor_range.primary.index.max(cursor_range.secondary.index);
                ...
                if state.hover_edit_enabled {
                    ...
                    if is_hovering_sub_text {
                        ...
                        let is_dragged = out_sub.response.dragged();
                        if is_dragged {
                            let _ = _window.drag_window();
                            ui.ctx().memory_mut(|m| m.surrender_focus(edit_id));
                        } else if ui.ctx().dragged_id().is_none() {
            ...
            ui.add_space(10.0); // bottom padding
                ...
                // Dynamic height mode when editing - expands with content but with max height to prevent freeze
                let max_edit_height = ui.available_height() * 0.8; // Use 80% of available height
                egui::ScrollArea::vertical()
                    .max_height(max_edit_height)
                    .auto_shrink([false, false])
                    .show(ui, |ui| {
                        ui.set_width(card_width - 32.0);
                        ui.add_space(10.0);
            ...
            ui.add_space(10.0); // bottom padding
                    }); // Close ScrollArea for dynamic height mode
            }
        },
    );
...
fn render_single_quote_mode(
    ...
    ui.vertical_centered(|ui| {
        ...
        // Check if text is being dragged (for window move)
        let is_dragging = out.response.dragged();
        ...
        if state.hover_edit_enabled {
            // HOVER-TO-EDIT MODE: Auto-focus on hover with cursor at mouse position
            if is_hovering_main && !is_dragging {
                ui.ctx().set_cursor_icon(egui::CursorIcon::Text);
        ...
        if !state.quotes[idx].sub_text.is_empty() {
            ...
            // Check if text is being dragged (for window move)
            let is_dragging_sub = out_sub.response.dragged();
            ...
            if state.hover_edit_enabled {
                // HOVER-TO-EDIT MODE: Auto-focus on hover with cursor at mouse position
                if is_hovering_sub && !is_dragging_sub {
                    ui.ctx().set_cursor_icon(egui::CursorIcon::Text);
        ...
        .show(ctx, |ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    // Click on empty area clears input fields
                    ...
                    if has_editing {
                        ...
                        ui.vertical(|ui| {
                            ...
                            for idx in 0..state.quotes.len() {
                                ...
                                // Use hover-only interaction to NOT intercept clicks
                                // This allows text widgets inside the card to handle their own click/double-click events
                                let _card_interact = ui.interact(card_response.rect, card_id, Sense::hover());
                                
                                // Single-click on card now loads text into input fields for editing
                                // (This works because we no longer intercept clicks with Sense::click())
                                if card_response.rect.contains(ui.ctx().pointer_interact_pos().unwrap_or(Pos2::new(-1000.0, -1000.0))) {
                                    if ui.input(|i| i.pointer.button_pressed(egui::PointerButton::Primary)) {
                                        ...
                                        state.sub_text_input = state.quotes[idx].sub_text.clone();
                                        state.editing_quote_index = Some(idx);
                                        // Request focus on the main text edit field to make it immediately editable
                                        ctx.memory_mut(|m| m.request_focus(card_id.with("edit_main")));
                                    }
                                }
            ...
            render_section(ui, &format!("ADD CUSTOM TEXT  [{}]", state.quotes.len() + 1), u32_to_color32(state.text_style.panel_text_color), |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    if text_response.changed() {
                        // Autosave on every keypress
                        if let Some(edit_idx) = state.editing_quote_index {
                            // Editing existing quote - update it
                        ...
                        .show(ui, |ui| {
                            let resp = ui.add(
                                egui::TextEdit::multiline(&mut state.sub_text_input)
                                    .hint_text(
                                        "Supporting text... (Enter for new line)",
                                    )
                                    .desired_rows(2)
                                    .desired_width(text_width)
                                    .frame(false),
...
impl ApplicationHandler for AppRunner {
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        // Check if we need to redraw for animations or quote rotation
        ...
        if needs_redraw {
            if let Some(window) = self.window.clone() {
                self.render(window.as_ref());  // Clone Arc, then convert to &Window
            }
        }
...
impl AppRunner {
    fn render(&mut self, window: &Window) {
        ...
        let full_output = egui_ctx.run(raw_input, |ctx| {
            ...
            // Clear input fields if clicked outside of any text editing area
            if ctx.input(|i| i.pointer.any_click()) {
                // Check if any TextEdit has focus
                let has_text_focus = ctx.memory(|mem| mem.focused().is_some());
                
                // If no text field has focus and we're editing, clear the input fields
                if !has_text_focus && app_state.editing_quote_index.is_some() {
                    app_state.save();
